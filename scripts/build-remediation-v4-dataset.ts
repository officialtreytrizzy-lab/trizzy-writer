...
const SOURCE = path.join(ROOT, "training", "remediation_v4_examples.seed.json");
...
async function main() {
  ...
  for (const example of seed.examples) {
    ...
    example.prompts.forEach((userPrompt, index) => {
      ...
      rows.push({
        ...
        metadata: {
          source: "Trey AI behavioral remediation v4",
          seedId: example.id,
          promptIndex: index,
          approved: true,
        },
  ...
  const manifest = {
    schemaVersion: 1,
    source: "training/remediation_v4_examples.seed.json",
    sourceSha256: hash(sourceText),
    totalRows: rows.length,
    trainRows: train.length,
    evalRows: evaluation.length,
    purpose: "Focused continued SFT for remaining behaviors after revision-3 evaluation.",
  };

  if (!CHECK_ONLY) {
    await fs.mkdir(OUTPUT, { recursive: true });
    await fs.writeFile(path.join(OUTPUT, "remediation-v4-train.jsonl"), train.map((row) => JSON.stringify(row)).join("\n") + "\n");
    await fs.writeFile(path.join(OUTPUT, "remediation-v4-eval.jsonl"), evaluation.map((row) => JSON.stringify(row)).join("\n") + "\n");
    await fs.writeFile(path.join(OUTPUT, "remediation-v4-manifest.json"), JSON.stringify(manifest, null, 2) + "\n");
  }
